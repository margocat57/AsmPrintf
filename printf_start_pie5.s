...
; nasm -f elf64 -o asm5.o printf_start_pie5.s  
; gcc -fsanitize=address -std=c2x -o program5 test3.c asm5.o (sanitizer)
; gcc  -std=c2x -o program5 test3.c asm5.o

; objdump -M intel -d asm5.o | less
...
process_spec:       movzx esi, byte [rdi] 
                    ...
                    add r9, [r9 + 8*rsi]           ; r9 = (rip + offset FmtSpecTable)(in r9) + offset func (in rsi)
                    jmp r9
...
output_bin_hex_oct: test rsi, rsi
                    ...
                    mov r8d, 1
                    shl r8d, cl
                    sub r8d, 1    ; 2^cl - 1 (bin: 2^1 - 1, oct: 2^3-1, hex: 2^4-1 - маска для выцепления 1, 3, 4 битов)

                    movzx edx, cl 

                    xor ecx, ecx
                    bsr rcx, rsi                  ; старший единичный бит
                    movzx ecx, byte [r9 + rcx]    ; получаем с какого сдвига начнать (наименьшее число делящееся на 2^(величина сдвига))
